 ...
 /*   MouseHit.prototype.__createHammer = function() {
   ...
   /**
    ...
    MouseHit.prototype.__createMouse = function() {
	   ...
	   for(i=0;i<3;i++)
	   {
            ...
            for(j=0;j<3;j++)
			{
              ...
              mouse3.init('mouse4',this.mouseX[i][j],this.mouseY[i],106);
			  /*mouse4.init('mouse5',this.mouseX[i][j],this.mouseY[i]-25,130);*/
              arr[j] = mouse0,arr1[j] = mouse1,arr2[j] = mouse2,arr3[j] = mouse3;
			}
			this.mouse0[i] = arr,this.mouse1[i] = arr1,this.mouse2[i] = arr2,this.mouse3[i] = arr3;
		}
    }
    /**
     * @private
    ...
    MouseHit.prototype.__createScene = function() {
		...
		/*}else{
			this.bg_hole = new my.Bitmap({
				image : my.ImageManager.get('bg_hole1'),
				width : document.documentElement.clientWidth,
				height : document.documentElement.clientHeight/2+100
			});
		}*/
    ...
    MouseHit.prototype.init = function() {
		...
		if(this.wid<375){
			this.mouseY = [110,210,300];
		}else if(this.wid>400){
			this.mouseY = [160,280,400];
		}
    ...
    MouseHit.prototype.ready = function(self,index) {
		...
		self.__setIntervalFunc(self);
		/*switch(index){
		    
			case 0:self.context.drawImage(self.startNumber,449,296,51,87,360,300,51,87);break;    //绘制3
			case 1:self.context.drawImage(self.startNumber,390,296,54,87,360,300,54,87);break;    //绘制2
			case 2:self.context.drawImage(self.startNumber,329,296,52,87,360,300,52,87);break;    //绘制1
			case 3:self.context.drawImage(self.startNumber,15,296,293,87,245,300,293,87);break;   //绘制start
	...
	MouseHit.prototype.__drawReadyScreen = function(self){
	    ...
	    self.context.drawImage(self.bg_hole.image, 0, 0, self.bg_hole.width, self.bg_hole.height,0,0,this.wid,self.bg_hole.height);
	}
	/**
	 *计时条绘制
	 *passPix 过去像素
	...
	MouseHit.prototype.__drawTimeRoller = function(self,passPix){
		self.context.drawImage(self.timeRoller,0,80,373,68,10,10,220,40);
		self.context.drawImage(self.timeRoller,0,152,373-passPix,40,9+passPix,24,220-passPix,25);
	    self.context.drawImage(self.timeRoller,0,0,373,80,10,4,220,51);
	}
	/**
	 * 游戏主体在这里设置setInterval事件
	 */
	MouseHit.prototype.__setIntervalFunc = function(self){
		var speed=3;//地鼠出现的初始速度
	    this.drawCanvasInterval = setInterval(function(){
		   self.drawScreen(self);
		   self.__checkIsPass(self);   //时间控制
		}, 30);
	    this.drawMouseInterval = setInterval(function(){
			speed+=0.1;
		   self.randomSelectMouse(self,speed);
		}, 2000);//每两秒随机出现一个地鼠
	...
	MouseHit.prototype.__checkIsPass = function(self){
	    ...
	    if(self.timeCaculator>2100){             //关卡时间到
		    ...
		    /*if(self.ui.score>=self.requireScore){    //过关
				...
				self.dijiguan=1;*/
			self.ui.toOver();//失败界面
			my.DOM.get('score').innerHTML="成绩："+~~self.ui.score;
			self._gameover(self);
			self.ui.score=0;
	 ...
	 * 游戏结束，分数发送到服务端
	 */
	MouseHit.prototype._gameover = function(self){
		var game = { PlayerID: 1, GameID: 1, GameMissionID: 0, PlayTime: 140, Score: self.ui.score,BonusPoints:50, ExtraInfo: "zht" };

		$.ajax({
			url: "http://192.168.1.106:1997/game/receivedata",
			type: "POST",
	 ...
	 * 随机显现地鼠
	 */
	MouseHit.prototype.randomSelectMouse = function(self,speed){
		 var createNum = my.Math.random(1,1);	//随机显现地鼠个数
		 for(var i=1;i<=createNum;i++){
			 ...
			 if(self.existMatrix[a][b]==1){  // 存在地鼠，不产生地鼠
				...
				var k = my.Math.random(0,3);
				if(k==0){self.mouse0[a][b].state='normal';self.mouse0[a][b].duration = speed;}
				else if(k==1){self.mouse1[a][b].state='normal';self.mouse1[a][b].duration = speed;}
				else if(k==2){self.mouse2[a][b].state='normal';self.mouse2[a][b].duration = speed;}
				else{self.mouse3[a][b].state='normal';self.mouse3[a][b].duration = speed}
	...
	MouseHit.prototype.drawScreen = function(self){
	    ...
	    //self.context.drawImage(self.bg_hole.image, 0, 0, self.bg_hole.width, self.bg_hole.height,0,120,this.wid,self.bg_hole.height);   //背景，并不完全占满整个canvas，所以要重新清屏
        for(j=0;j<3;j++)
		  self.drawMouse(self,0,j); //第一行地鼠
		for(j=0;j<3;j++) 
		  self.star[0][j].draw(self.context);  //第一行星星
	 	self.context.drawImage(self.bg_holeHide.image,0,0,self.bg_holeHide.width,145, 0, 0, self.bg_holeHide.width,145);   //第一行遮掩洞
        for(j=0;j<3;j++)
		  self.drawMouse(self,1,j); //第二行地鼠
		for(j=0;j<3;j++) 
		  self.star[1][j].draw(self.context);  //第二行星星
        self.context.drawImage(self.bg_holeHide.image,0,145,self.bg_holeHide.width,77, 0, 145, self.bg_holeHide.width,77); //第二行遮掩洞
        for(j=0;j<3;j++)
		  self.drawMouse(self,2,j); //第三行地鼠
		for(j=0;j<3;j++) 
		  self.star[2][j].draw(self.context);  //第三行星星
		self.context.drawImage(self.bg_holeHide.image,0,190,self.bg_holeHide.width,77, 0, 190, self.bg_holeHide.width,77); //第三行遮掩洞
		for(i=0;i<3;i++) 
		  for(j=0;j<3;j++) 
