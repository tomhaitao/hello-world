   ...
   /* UI.prototype.__initBtnPause = function() {
	 ...
	 */
	 UI.prototype.__initMouseEventListener=function(){
	     var canvas =  my.DOM.get('maincanvas'),self = this;
	   /*  canvas.onmouseover = function(){
         ...
         canvas.addEventListener("touchstart",function(e){
             ...
             var touch = even.touches[0];
             //self.__pengzhuang(self,even.clientX-300, even.clientY-40);
             self.__pengzhuang(self,touch.clientX+50, touch.clientY-40);
         },false)
	     /*canvas.onmouseup = function(e){
		     self.mousePress = false;
			 var even = window.event || e;  
			 //self.__pengzhuang(self,even.clientX-300, even.clientY-40);
             self.__pengzhuang(self,even.clientX+50, even.clientY-40);
		 }*/
         canvas.addEventListener("touchend",function(e){
             self.mousePress = false;
            /* var even = window.event || e;
             var touch = event.touches[0];
             //alert("X坐标:"+touch.clientX);
             //self.__pengzhuang(self,even.clientX-300, even.clientY-40);
             self.__pengzhuang(self,touch.clientX+50, touch.clientY-40);*/
         },false)
	 }
     ...
     */
         ...
         for(i=0;i<3;i++){
           for(j=0;j<3;j++){
			     if(self.existMatrix[i][j]==1){
                    var it0 = self.mouse0[i][j],it1 = self.mouse1[i][j],it2 = self.mouse2[i][j],it3 = self.mouse3[i][j];
	 ...
	 UI.prototype.__boundTest=function(self,it,x,y,i,j){
		  if(it.state=='normal'){
                 if(x>(it.x-it.width) && y>(it.y-50) && x<(it.x+it.width+it.width) && y<(it.y+it.height-50)){
                      it.state = 'dead';
	 ...
	 */
	 UI.prototype.__caculateScore = function(self,type,x,y,i,j){
	      switch(type)
          {
             case "mouse0":self.scoreObject[i][j].scoreType=4; this.score+=score1; break;                          //冰激凌+200
             case "mouse1":self.scoreObject[i][j].scoreType=3; this.score+=score2;            break;                          //鸡腿+500
             case "mouse2":self.scoreObject[i][j].scoreType=0; this.score+=score3;            break;                          //蛋糕+400
             case "mouse3":self.scoreObject[i][j].scoreType=2; this.score-=score4;if(this.score<0) this.score=0; break;       //棒棒糖-100
            /* case "mouse4":self.scoreObject[i][j].scoreType=1; this.score/=2;              break;          */                //x/2
          }
		   self.scoreObject[i][j].x=x-80;self.scoreObject[i][j].y=y;
